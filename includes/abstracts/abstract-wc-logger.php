 ...
 * @since      1.0.0
 *
 * @package    Max_Marine_Edit_Product_Shared_State/includes/abstracts
 */
 ...
 *
 * @package  Max_Marine\Edit_Product_Shared_State\Core\Abstracts\Max_Marine_Edit_Product_Shared_State_Abstract_WC_Logger
 * @author   David Baumwald <david@dream-encode.com>
 */
class Max_Marine_Edit_Product_Shared_State_Abstract_WC_Logger {
	...
	public static function log( $data, $level = 'debug' ) {
		...
		$log_level_setting = max_marine_edit_product_shared_state_get_plugin_setting( 'log-level', 'off' );
		...
		if ( ! function_exists( 'wc_get_logger' ) ) {
			...
			error_log(
				sprintf(
					...
					__( '%1$s->%2$s depends on `wc_get_logger` which is bundled with the WooCommerce plugin.', 'max-marine-edit-product-shared-state' ),
					__CLASS__,
					__METHOD__
				)
			);
